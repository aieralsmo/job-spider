...
class ZhaopinspiderSpider(scrapy.Spider):
    ...
    def parse_city(self, response):
        ...
        # 解析数据
        for data in resp_dict['data']['results']:
            # TODO: 解析数据，只取我们需要的信息
            item = ZhaopinItem()

            item['job'] = data.get("jobName")
            item['company'] = data.get("company").get("name")
            item['address'] = data.get("city").get("display")
            item['salary'] = data.get("salary")
            item['publish_date'] = data.get("updateDate")
            item['eduLevel'] = data.get("eduLevel").get("name")
            item['workingExp'] = data.get("workingExp").get("name")
            item['positionURL'] = data.get("positionURL")
